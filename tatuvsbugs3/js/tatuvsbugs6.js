...
/*
		function loadHandler() {
			assetsLoaded++;
			if (assetsLoaded == spritesImagenes.length) {
...
*/
		...
		function cleanStage() {
			...
			for (var i = spritesImagenes.length; i > 0; i--) {
				spritesImagenes.pop();
			}
		}
		...
		function loadMap(mapToLoad) {
			...
			var ROWS = mapToLoad.length;
			var COLS = mapToLoad[0].length;
			var xCor, yCor;
			var SIZE_UNITY = 32;
			var characterToCreate;

			for (var row = 0; row < ROWS; row++) {
				for (var col = 0; col < COLS; col++) {
					characterToCreate = mapToLoad[row][col];
					if (characterToCreate != NOTHING_TYPE) {
						//count to assets
//						assetsLoadedTotal++;
						xCor = Math.floor(col * SIZE_UNITY);
						yCor = Math.floor(row * SIZE_UNITY);
						createCharacters(characterToCreate, xCor, yCor);
					}//distinct
				}
			}
		}//loadMap()
		...
		function loadLevel(levelNumber) {
			//reset Stage CounterEnemies
			counterEnemies = 0;
			//clean array to begin new stage
			cleanStage();
			//========================================================
			...
			//save object player data
			sprites.push(objectPlayer);
			//==================load images  ========
			//Backgroung image
			imagen = new Image();
			imagen.src = PATH_IMAGES + levelNumber + ".png";
...
//					imagen.addEventListener("load", loadHandler, false);
				...
				case POWER_COIN:
					...
					objectCollide.x = xPos;
					objectCollide.y = yPos;
					objectCollide.type = POWER_COIN;
					objectCollide.mobile = false;
					objectCollide.strength = 1;
					sprites.push(objectCollide);
					//=============================================================
					imagen = new Image();
					imagen.src = PATH_IMAGES + IMAGE_POWER_COIN;
					imagen.addEventListener("load", loadHandler, false);
					spritesImagenes.push(imagen);
					break;
				case WALL_TYPE:
					...
					objectCollide.x = xPos;
					objectCollide.y = yPos;
					objectCollide.type = WALL_TYPE;
					objectCollide.mobile = false;
					objectCollide.strength = 1;
					objectCollide.totalFrames = 0;
					sprites.push(objectCollide);
					//=============================================================
					imagen = new Image();
					imagen.src = PATH_IMAGES + IMAGE_WALL_TYPE;
					imagen.addEventListener("load", loadHandler, false);
					spritesImagenes.push(imagen);
//					image.removeEventListener("load", loadHandler, false);
					break;
		...
		function EndGame(objectToPrint) {
			...
			if (objectToPrint.enemiesDefeat == counterEnemies) {
				if (levelActual < TOTAL_LEVELS) {
					levelActual++;
					message.text = "STAGE" + levelActual;
					//					cleanStage();
					gameState = LOADING_MAP;
					//remove message from screen
					setTimeout(removeMessageScreen, removeMessageTime);
				} else {
							...
							if (i != j) {
								if (sprites[j].visible) {
									...
									if (hitTestCircle(sprites[i], sprites[j])) {
										...
										//only block them
										if ((sprites[i].type == BUG_TYPE1 || sprites[i].type == BUG_TYPE2 || sprites[i].type == BUG_TYPE3) && (sprites[j].type == BUG_TYPE1 || sprites[j].type == BUG_TYPE2 || sprites[j].type == BUG_TYPE3 || sprites[j].type == WALL_TYPE)) {
											blockCircle(sprites[i], sprites[j]);
											sprites[i] = InteligenciaBug(sprites[i]);											
										}//
										//=========================================================
										//===================== RULES OF COLLISIONS ===============
										...
										if ((sprites[i].type == PLAYER_TYPE) && (sprites[j].type == BUG_TYPE1 || sprites[j].type == BUG_TYPE2 || sprites[j].type == BUG_TYPE3)) {
											if (moveSpaceFlag) {
												...
												if (sprites[i].enemiesDefeat == counterEnemies) {
													gameState = OVER_STATE;
												}
											} else {
										...
										if ((sprites[i].type == PLAYER_TYPE) && (sprites[j].type == POWER_COIN)) {
											//add score
											score++;
											DeleteBug(sprites[j], sprites, spritesImagenes[j], spritesImagenes);
										}//

										if ((sprites[i].type == PLAYER_TYPE) && (sprites[j].type == WALL_TYPE)) {
											//add score
											//apply the rules to colliders PLAYER vs enemies, player defeat
											blockCircle(sprites[i], sprites[j]);
		...
		function PrintInformation(objectToPrint) {
			var enemiesDefeatPrint = document.getElementById("score");
			var strengthPrint = document.getElementById("strength");
			var totalEnemiesPrint = document.getElementById("totalEnemies");
			...
			var scorePrint = document.getElementById("scorePoints");

			enemiesDefeatPrint.innerHTML = objectToPrint.enemiesDefeat;
			strengthPrint.innerHTML = objectToPrint.strength;
			totalEnemiesPrint.innerHTML = counterEnemies;
			...
			levelPrint.innerHTML = levelActual;
