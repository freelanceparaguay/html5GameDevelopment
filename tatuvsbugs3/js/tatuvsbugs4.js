...
//					createCharacters(POWER_ARMOR, Math.floor(Math.random() * objectStage.width), Math.floor(Math.random() * objectStage.height));
		...
		//			console.log("cantidad sprites="+sprites.length);
		//			if (sprites.length !== 0) {
		//				for(i=1; i<sprites.length;i++){
		//					sprites.pop();
		//				}
		//			}
		//			if (spritesImagenes.length !== 0) {
		//				for(i=1; i<spritesImagenes.length;i++){
		//					spritesImagenes.pop();
		//				}
		//			}
		...
		function EndGame(objectToPrint) {
			...
			if (objectToPrint.enemiesDefeat == counterEnemies) {
				if (levelActual <= TOTAL_LEVELS) {
					...
					setTimeout(removeMessageScreen, 1000);
				} else {
									...
									if (hitTestCircle(sprites[i], sprites[j])) {
										...
										if ((sprites[i].type == PLAYER_TYPE) && (sprites[j].type == BUG_TYPE1 || sprites[j].type == BUG_TYPE2 || sprites[j].type == BUG_TYPE3)) {
											if (moveSpaceFlag) {
												...
												//												console.log("enemies="+sprites[i].enemiesDefeat);
												sprites[i].addScore();
